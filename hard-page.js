...
document.addEventListener('DOMContentLoaded', () => {
  ...
  var boxes = document.querySelectorAll('.box-1-hard');
  const timerElement = document.querySelector('#timer-hard');
  const scoreElement = document.querySelector('#score-hard');
  ...
  let timeRemaining = 100; // Set the time limit for the game (in seconds)
  ...
  function gameCompleted() {
    ...
    window.location.href = './hard-game-over.html';
  }

window.onload = function () {
  var boxesContainer = document.querySelector(".boxes-hard");
...
function randomizeBoxes(container) {
  var boxes = container.querySelectorAll(".box-1-hard");
  ...
  function startGame() {
    gameStarted = true;
    timeRemaining = 100; // Reset the time when starting the game
